...
int *mode(const int *array, int arySize) {
    ...
    for (int i = 1; i < arySize; ++i) {
        if (ary[i] == ary[i - 1]) {
            ...
            currentFreq = 1; // Reset frequency counter
        }
    }
    
    // Step 4: Count the number of modes with max frequency
    int numModes = 0;
    currentFreq = 1;
    ...
    // Step 5: Allocate memory for the mode array
    int *modeAry = new int[numModes + 2]; // Size is 2 + number of modes
    modeAry[0] = numModes; // Number of modes
    modeAry[1] = maxFreq;  // Max frequency
    
    // Step 6: Fill the mode array with modes
    currentFreq = 1;
    int modeIndex = 2;
    ...
    // Step 7: Clean up and return the mode array
    delete [] ary;
    return modeAry;
}
